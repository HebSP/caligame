...
function criarSetas(ligações, container, camadas) { 
    ...
    ligações.forEach(ligação => {
        ...
        path.setAttribute("d", `M ${inicioX} ${inicioY} C ${inicioX} ${midY} , ${fimX} ${midY} , ${fimX} ${fimY}`);
        ...
        if (camadaFim === camadaInicio + 1) { // se a ligação é entre camadas adjacentes
            ...
            path.setAttribute("stroke-dasharray", "5,5"); // linha tracejada para indicar salto de camada
...
function addMeios(camadas, meios) { /////AAAAAAAAAAAAAAAAAAAAAAAAA n sei mais como ordenar isso
    meios.forEach((intermediarios, key) => {
        const [keyInicio, keyFim] = key.split('-to-');
        const inicio = document.getElementById(keyInicio);
        const fim = document.getElementById(keyFim);
        const camadaInicio = getCamadaIndex(inicio, camadas);
        const camadaFim = getCamadaIndex(fim, camadas);
        for (let i = camadaInicio + 1; i < camadaFim; i++) {
            if (!camadas[i]) camadas[i] = [];
            // posição relativa media entre o inicio e o fim
            intermediarios[i-camadaInicio-1].posicaoRelativa = inicio.posicaoRelativa + (fim.posicaoRelativa - inicio.posicaoRelativa)* (i - camadaInicio)/ (camadaFim - camadaInicio)
            // index dentro da camada (avaliar melhor depois)
            intermediarios[i-camadaInicio-1].index = camadas[i]/2 + (intermediarios[i-camadaInicio-1].posicaoRelativa)+ 0.1
    ...
    habilidades.forEach(habilidade => { 
        ...
        habilidade.pre_requisitos.forEach(req => {
            ...
            /*if (pai) {
                ...
                let tempCamadaPai = camadaPai;
                while (tempCamadaPai < camadaHabilidade - 1) {
                    // ligação entre camadas não adjacentes, precisa de um meio
                    meios[tempCamadaPai].push([pai.node, habilidade.node]);
                    tempCamadaPai++;
                }
            ...
            if (pai) {
                ...
                if (camadaPai + 1 === camadaHabilidade) {
                    ...
                    let tempPai = pai;
                    meios[`intermediario-${tempPai.key}-to-${habilidade.key}`] = [];
                    while (camadaPai + 1 < camadaHabilidade) {
                        ...
                        meios[`intermediario-${tempPai.key}-to-${habilidade.key}`].push(nodoIntermediário);
